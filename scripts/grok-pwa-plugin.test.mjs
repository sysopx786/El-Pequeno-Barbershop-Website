...
test("this project's site.json title wins over a document <title>", () => {
  const out = injectGrokPwaHead(
    "<html><head><title>Hello World</title></head></html>",
    { cwd: TEMPLATE_ROOT },
  );
  const ogTitle = out.match(/property="og:title" content="([^"]*)"/)?.[1];
  assert.equal(ogTitle, SITE_TITLE);
  assert.notEqual(ogTitle, "Hello World");
});
